...
describe("Get Comments by PostId route", () => {
  //DB Connection Setup
  ...
  it("should return notFound Error upon no token is passed in the Header.Authorization", async () => {
    //Test
    const response = await request(app)
      .get(`/api/post/comment/${user1PostId}`)
  ...
  //Test - Post Exist
  it("should return NotFoundError upon no post exist for the post Id", async () => {
  ...
  //Test - Get comments
  it("should return all comments for the given post Id", async () => {
    //Test
    const response = await request(app)
      .get(`/api/post/comment/${user1PostId}`)
      .set("Authorization", `Bearer ${user1Token}`)
    ...
    const expectedResponse = {
      ...
      Comments: expect.arrayContaining([expect.anything() || null]),
    };
    //Assertions
    expect(response.status).toBe(200);
    expect(response.body).toMatchObject(expectedResponse);
